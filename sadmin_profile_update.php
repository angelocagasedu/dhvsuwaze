...
if(isset($_POST['update'])){
   ...
   $update_profile->execute([$name, $email, $sadmin_id]);
   ...
   if(!empty($image)){
      ...
      if($image_size > 2000000){
         ...
         $update_image->execute([$image, $sadmin_id]);
   ...
   if(!empty($previous_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($previous_pass != $old_pass){
         ...
         $update_password->execute([$confirm_pass, $sadmin_id]);
...
<head>
   ...
   <title>sadmin profile update</title>
...
<h1 class="title"> update <span>sadmin</span> profile </h1>

<section class="update-profile-container">

   <?php
      $select_profile = $conn->prepare("SELECT * FROM `users` WHERE id = ?");
      $select_profile->execute([$sadmin_id]);
